// BlackJack.Realtime/Hubs/SpectatorHub.cs - CORREGIDO CON auto-betting integrado
...
public class SpectatorHub : BaseHub
{
    ...
    private async Task<RoomInfoModel> MapToRoomInfoAsync(BlackJack.Domain.Models.Game.GameRoom room)
    {
        try
        {
            _logger.LogInformation("[SpectatorHub] Mapping room {RoomCode} to RoomInfoModel with auto-betting data", room.RoomCode);
            ...
            _logger.LogInformation("[SpectatorHub] Auto-betting calculation: MinBetPerRound={MinBet}, SeatedPlayers={SeatedCount}, Active={Active}",
            ...
            _logger.LogInformation("[SpectatorHub] RoomInfoModel created successfully for room {RoomCode} with {PlayerCount} players, MinBetPerRound={MinBet}",
            ...
            _logger.LogInformation("[SpectatorHub] Player balance details: {Players}", playerDetails);

            return roomInfo;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[SpectatorHub] Error mapping room {RoomCode} to RoomInfoModel with auto-betting data", room.RoomCode);
            throw;
        }
    }
